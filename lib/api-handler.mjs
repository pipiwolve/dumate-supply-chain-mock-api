...
export async function handler(req, res) {
  try {
    ...
    if (pathname === "/api/mcp" && req.method === "POST") {
      ...
      if (payload.method === "initialize") return sendJson(res, { jsonrpc: "2.0", id: payload.id ?? null, result: { protocolVersion: "2025-06-18", capabilities: { tools: {} }, serverInfo: { name: "dumate-supply-chain-systems", version: "0.2.0" } } });
      if (payload.method === "tools/list") return sendJson(res, { jsonrpc: "2.0", id: payload.id ?? null, result: { tools: [
      ...
      if (payload.method === "tools/call") return sendJson(res, { jsonrpc: "2.0", id: payload.id ?? null, result: { content: [{ type: "text", text: JSON.stringify(await mcpCall(payload.params?.name, payload.params?.arguments || {})) }] } });
