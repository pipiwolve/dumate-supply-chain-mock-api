...
export async function handler(req, res) {
  try {
    ...
    if ((pathname === "/api/mcp" || pathname === "/api/mcp/messages") && req.method === "OPTIONS") {
      res.statusCode = 204;
    ...
    if (pathname === "/api/mcp" && req.method === "GET") {
      // Keep the negotiation stream alive long enough for clients to discover
      // the endpoint and issue initialize/tools/list on the same session.
      const sessionId = req.headers?.["mcp-session-id"] || `demo-${Date.now().toString(36)}`;
      res.statusCode = 200;
      res.setHeader("Content-Type", "text/event-stream; charset=utf-8");
      res.setHeader("Cache-Control", "no-store");
      ...
      res.setHeader("MCP-Protocol-Version", "2025-06-18");
      res.setHeader("Access-Control-Allow-Origin", "*");
      const origin = `https://${req.headers.host || "www.demofun.online"}`;
      res.write(`event: endpoint\ndata: ${origin}/api/mcp/messages?sessionId=${encodeURIComponent(sessionId)}\n\n`);
      const heartbeat = setInterval(() => res.write(`: heartbeat ${Date.now()}\n\n`), 15000);
      const closeStream = () => clearInterval(heartbeat);
      req.on("close", closeStream);
      setTimeout(() => { clearInterval(heartbeat); if (!res.writableEnded) res.end(); }, 55000);
      return;
    }
    if (pathname === "/api/mcp/messages" && req.method === "POST") {
